...
def _build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument(
        ...
        default=None,
        metavar="N",
        help="MPI processes (skips the interactive prompt)",
    )
    p.add_argument(
        "--elastic", action="store_true", help="Force-enable elastic constants"
    )
    p.add_argument(
        "--no-elastic", action="store_true", help="Force-skip elastic constants"
    )
    p.add_argument(
        "--engine-cmd",
        dest="engine_cmd_override",
        metavar="CMD",
        help="Override the engine binary command",
    )
    p.add_argument(
        "--mode",
        choices=["vca", "sqs", "direct"],
        default="vca",
        help="Crystal preparation mode",
    )
    return p
...
def _detect_engine(args: argparse.Namespace) -> str:
    ...
    if not ENGINES:
        sys.exit(
            "  Fatal: No engines registered. Check engines/ directory and @register_engine decorators."
        )
    ...
    ui.section("Engines")
    results = discover_engines()
    for name, err in results.items():
        if name.lower() in ENGINES or name in ENGINES:
            status = "(loaded)"
        elif err:
            # Show first line of traceback for quick diagnosis
            first_line = (
                err.strip().splitlines()[-1] if err.strip() else "unknown error"
            )
            status = f"(error: {first_line})"
        else:
            status = "(not found)"
...
def _try_resume(work_dir: Path, stem: str, args: argparse.Namespace) -> RunState | None:
    """
    Автоматично шукає останній розрахунок для цього файлу.
    Якщо він не завершений — пропонує продовжити.
    """
    ...
    # Якщо всі кроки вже виконані успішно, немає сенсу пропонувати відновлення!
    if state.n_done == len(state.steps):
        return None

    # Скидаємо статус 'running' на 'pending', якщо процес був жорстко вбитий (напр. вимкнули світло)
    for step in state.steps:
        if step.status == RUNNING:
            step.status, step.rc = PENDING, "interrupted"

    print(f"\n  ℹ  Found incomplete run : {state.proj_dir.name}")
    print(
        f"     System: {state.system_label()} | Progress: {state.n_done}/{len(state.steps)} steps done"
    )

    # Запитуємо користувача, НАВІТЬ якщо він не передав прапорець --resume
    if ui.ask_yes_no("  Do you want to resume this calculation?", default=True):
        save_run(state)
        return state
...
def _run_sweep(
    ...
    *,
    ...
    for step in state.steps:
        ...
        ui.print_step_result(
            result, step, state.proj_dir, state.seed, state.run_elastic
        )
    ...
    # Smart retry — capability check via isinstance (never hasattr)
    if isinstance(engine, RecoveryCapable):
        failed = [s for s in state.steps if s.status == FAILED]
        if failed:
            retry_info = engine.retry_schema()
            for s in ui.print_smart_retry(failed, state):
                step_dir = state.proj_dir / s.step_dir
                reason = s.parsed.get("kill_reason", "")
                engine.patch_for_recovery(step_dir, state.seed, reason)
                ui.print_step_header(s, n_total, state)
                res = execute_step(state, s, crystal, engine, keep_all)
                ui.print_step_result(
                    res, s, state.proj_dir, state.seed, state.run_elastic
                )
...
def main() -> None:
    ...
    try:
        ...
        print(
            f"  Structure: {crystal.num_atoms} atoms (primitive cell) loaded via spglib."
        )
    except Exception as e:
    ...
    if state is None:
        if engine_name not in ENGINES:
            sys.exit(
                f"  ERROR: Engine '{engine_name}' not registered. Available: {list(ENGINES)}"
            )
        ...
        cli_species = (
            []
            if args.single
            else ([e.capitalize() for e in args.species] if args.species else None)
        )

        override = getattr(args, "engine_cmd_override", None)

        # setup_interactive is guaranteed by BaseEngine ABC — no hasattr guard needed
        engine, engine_cmd = engine_cls.setup_interactive(src, crystal, override, args)

        if getattr(engine, "skip_vca_wizard", False):
            # MLIP/direct engines set skip_vca_wizard=True and provide their own wizard result
        ...
        # Serialize via to_dict() — Path objects get {"__type__": "path", "val": ...}
        serializable_kwargs = engine.to_dict()
        ...
        state = new_run(
            ...
            crystal_mode=args.mode,
        )
    else:
        # Resume: validate engine_name consistency
        if state.engine_name and state.engine_name != engine_name:
            print(
                f"  ⚠  Warning: run was created with '{state.engine_name}' "
                f"but current engine is '{engine_name}'. Using '{state.engine_name}'."
            )
        ...
        try:
            engine = engine_cls.from_dict(kwargs)
        except Exception as exc:
            print(
                f"  ⚠  Could not restore engine from saved kwargs ({exc}), re-running setup..."
            )
            override = getattr(args, "engine_cmd_override", None)
    ...
    _run_sweep(
        state, crystal, engine, interactive=args.interactive, keep_all=args.keep_all
    )
